...
public class Chunk {
            ...
            for (byte x = 0; x < chunkW; x++)
                for (byte z = 0; z < chunkL; z++) {
                    for (byte y = 0; y < SubChunk.chunkH; y++) {
                        ...
                        terrainData[SubChunk.POS_BLOCK_IDS + newIndex] = data[POS_BLOCK_IDS + oldIndex];
                        if (terrainData[SubChunk.POS_BLOCK_IDS + newIndex] != 0) emptySubChunkFlag = false;
                        NibbleOperation.writeNibble(terrainData, NibbleOperation.readNibble(data, oldIndex, POS_META_DATA), newIndex, SubChunk.POS_META_DATA);
                        NibbleOperation.writeNibble(terrainData, NibbleOperation.readNibble(data, oldIndex, POS_SKY_LIGHT), newIndex, SubChunk.POS_SKY_LIGHT);
                        NibbleOperation.writeNibble(terrainData, NibbleOperation.readNibble(data, oldIndex, POS_BLOCK_LIGHT), newIndex, SubChunk.POS_BLOCK_LIGHT);
                    }
                }
    ...
    public CompoundTag toTag() {
        ...
        tag.put(new IntTag("DataVersion", NBTVersion));

        CompoundTag level = new CompoundTag("Level", new ArrayList<>());
        level.put(new IntTag("xPos", chunkX));
        level.put(new IntTag("zPos", chunkZ));
        level.put(new LongTag("LastUpdate", System.currentTimeMillis())); //maybe 0?
        level.put(new ByteTag("LightPopulated", lightPopulated ? (byte) 1 : (byte) 0));
        level.put(new ByteTag("TerrainPopulated", terrainPopulated ? (byte) 1 : (byte) 0));
        level.put(new ByteTag("V", (byte) 1));
        level.put(new LongTag("InhabitedTime", 0));
        level.put(new ByteArrayTag("Biomes", biomeID));
        level.put(new IntArrayTag("HeightMap", heightMap));
        level.put(new ByteTag("TerrainGenerated", (byte) 1));

        ListTag sections = new ListTag("Sections", new ArrayList<>());
        subChunks.forEach((SubChunk subChunk) -> sections.put(subChunk.toTag()));

        level.put(sections);
        level.put(new ListTag("Entities", entities));
        level.put(new ListTag("TileEntities", tileEntities));
